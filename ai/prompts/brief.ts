...
export function buildRegionBriefPrompt(regionName: string, events: Array<{ title: string; summary: string; riskLevel: string; publishedAt: string }>): string {
  const eventList = events
    .map((e, i) => `${i + 1}. [${e.riskLevel.toUpperCase()}] ${e.title} (${e.publishedAt})\n   ${e.summary}`)
    .join('\n\n');
